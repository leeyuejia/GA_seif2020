...
`fetch` is a function. it takes two arguments where the first one is the url and the second one **MUST** be an object. We'll code this out together after we put together our query string
